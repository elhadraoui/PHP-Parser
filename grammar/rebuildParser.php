    ...
    return preg_replace_callback(
        ...
        function($matches) {
            ...
            if ('parseEncapsed' == $name) {
                ...
                return 'foreach (' . $args[0] . ' as &$s) { if (is_string($s)) { $s = String::parseEscapeSequences($s, ' . $args[1] . '); } }';
            }

            if ('parseEncapsedDoc' == $name) {
                assertArgs(1, $args, $name);

                return 'foreach (' . $args[0] . ' as &$s) { if (is_string($s)) { $s = String::parseEscapeSequences($s, null); } } $s = preg_replace(\'~(\r\n|\n|\r)$~\', \'\', $s); if (\'\' === $s) array_pop(' . $args[0] . ');';
            }
